...
@test "rSerBase_06 - Create simple geo index and add entry" {
    ...
    ## add entry(s)
    run $command_base FT.ADD $index_name 1 1 FIELDS name "Monterrey" location -100.4431802,25.6490376
    [ "$status" -eq 0 ]
    [ "$output" = "OK" ]

    run $command_base FT.ADD $index_name 2 1 FIELDS name "Vancouver" location -123.1207,49.2827
    [ "$status" -eq 0 ]
    [ "$output" = "OK" ]

    run $command_base FT.ADD $index_name 3 1 FIELDS name "London" location -0.1278,51.5074
    [ "$status" -eq 0 ]
    [ "$output" = "OK" ]

    ## get index content without filters
    run $command_base FT.SEARCH $index_name "*"
    [ "$status" -eq 0 ]
    [ "${lines[1]}" -eq 3 ] ## check number of expected returns

    ## search index by single field
    run $command_base FT.SEARCH $index_name "@name:Lo*"
    [ "$status" -eq 0 ]
    [ "${lines[3]}" == "London" ]

    ## search index by geospatial field returning only a single field
    run $command_base FT.SEARCH $index_name "@location:[0 51.5 50 km]" RETURN 1 name
    [ "$status" -eq 0 ]
    echo $output
    [ "${lines[3]}" == "London" ]

    ## search index by geospatial and text field returning only a single field
    run $command_base FT.SEARCH $index_name "@name:Van* @location:[-120 50 500 km]" RETURN 1 name
    [ "$status" -eq 0 ]
    [ "${lines[3]}" == "Vancouver" ]
